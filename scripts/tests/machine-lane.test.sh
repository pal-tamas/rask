...
assert_eq "the browser SQLite gate is a gate of its own" \
  "$(rask_lane_script_of 'bash scripts/run-browser-sqlite-e2e-local.sh')" "run-browser-sqlite-e2e-local.sh"
assert_eq "the data demo browser gate is a gate of its own" \
  "$(rask_lane_script_of 'bash scripts/run-data-demo-e2e-local.sh')" "run-data-demo-e2e-local.sh"
