...
for f in \
    ...
    llms.txt \
    src/Rask.Site/Features/Home/InstallTabs.cs \
    src/Rask.Site/Features/Guides/GuidesIndexPage.cs; do
...
check "both sites scaffold with the same command" yes \
    "$(grep -qF 'rask new MyApp' src/Rask.Site/Features/Home/InstallTabs.cs \
       && grep -qF 'rask new MyApp' src/Rask.Site/Features/Guides/GuidesIndexPage.cs \
