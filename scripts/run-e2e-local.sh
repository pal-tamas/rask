...
# The wait itself, and what it prints: shared with the devtools gate, so the two can never disagree about
# when a browser suite may start.
# shellcheck source=lib/e2e-admission.sh
. "$root/scripts/lib/e2e-admission.sh"

# RASK_E2E_QUEUE=0 is answered before the build, where refusing still saves it (scripts/lib/e2e-admission.sh).
