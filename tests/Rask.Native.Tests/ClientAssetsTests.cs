...
public class ClientAssetsTests
{
    ...
    public void ClientJs_HasNoUnreplacedSpliceMarkers()
    {
        ...
        string[] markers =
        [
            "// @@RASK_DOM@@", "// @@RASK_MORPH@@", "// @@RASK_API@@", "// @@RASK_EVENTS@@",
            "// @@RASK_PWA@@", "// @@RASK_INPUT@@", "// @@RASK_SCOPED@@"
        ];
    ...
    [Fact]
    public void ClientJs_SplicesTheSharedInputScopedAndKeyboardDragModules()
    {
        // Client-parity: the native dialect must reach the same transport-neutral DOM behaviour as
        // rask.js / rask.wasm.js by splicing the shared modules rather than re-copying them —
        // rAF input/scroll coalescing (rask-input.js), scoped-CSS FOUC gating (rask-scoped.js) and
        // keyboard + core drag (folded into rask-events.js).
        var js = NativeClientAssets.ClientJs;
        // rask-input.js — rAF input/scroll coalescing.
        Assert.Contains("function flushInputsNow", js);
        Assert.Contains("function queueInput", js);
        Assert.Contains("function flushScroll", js);
        // rask-scoped.js — scoped-CSS FOUC gating.
        Assert.Contains("function waitForUnappliedHeadCss", js);
        Assert.Contains("function preloadNewHeadStylesheets", js);
        // rask-events.js — keyboard + the four core drag events.
        Assert.Contains("data-rask-on-keydown", js);
        Assert.Contains("data-rask-on-dragstart", js);
    }
