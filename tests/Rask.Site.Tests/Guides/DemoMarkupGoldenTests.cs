...
public sealed class DemoMarkupGoldenTests
{
    ...
    public async Task No_demo_skeleton_changes_on_a_timer()
    {
        ...
        {
            var page = Page.Render(() => DemoRegistry.Build(key), TestServices.Default());
    ...
    public async Task The_demos_that_settle_late_keep_their_skeleton_across_the_settle()
    {
        ...
        var pages = keys
            .Select(key => (key, page: Page.Render(() => DemoRegistry.Build(key), TestServices.Default())))
            .ToList();
    ...
    private static string Skeleton(string key) =>
        SkeletonOf(Page.Render(() => DemoRegistry.Build(key), TestServices.Default()).Html);
