...
public sealed class UiKitWiringTests
{
    ...
    // its palette copy fails here rather than on a deployed page.
    //
    // One entry, where there used to be two: the landing page and the showcase were separate apps with
    // separate stylesheets, and each needed its own copy of the palette. They are one app now, so there
    // is one copy to keep honest. Kept as a Theory rather than collapsed to a Fact — the next app that
