...
#pragma warning disable RASK014 // the tab, its receiver and components made by hand, the way the panel page and a walk would
    private static Page Render(DevToolsFeed feed) => Page.Render(new DevToolsPerfTab { Feed = feed });

    private static Page Receiver(DevToolsFeed feed) => Page.Render(new DevToolsPatchReceiver { Feed = feed });
