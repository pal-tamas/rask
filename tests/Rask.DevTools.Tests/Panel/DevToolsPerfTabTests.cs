...
#pragma warning disable RASK014 // the tab, its receiver and components made by hand, the way the panel page and a walk would
    private static Page Render(DevToolsFeed feed) => Test.Render(new DevToolsPerfTab { Feed = feed });

    private static Page Receiver(DevToolsFeed feed) => Test.Render(new DevToolsPatchReceiver { Feed = feed });
