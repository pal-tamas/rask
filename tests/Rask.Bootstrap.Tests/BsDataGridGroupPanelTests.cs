...
public partial class BsDataGridGroupPanelTests : global::Rask.Core.RaskMarkup
{
    ...
    public async Task DraggingAHeaderOntoThePanel_GroupsByIt()
    {
        ...
        var repDragStart = Action(grid.Html, "dragstart", 1); // [0] is Region's header
        await grid.InvokeAsync(repDragStart);
        await grid.InvokeAsync(Action(grid.Html, "drop"));    // [0] is the panel itself
    ...
    public async Task DroppingOnAChip_InsertsBeforeIt()
    {
        ...
        await grid.InvokeAsync(Action(grid.Html, "dragstart", 1));
        await grid.InvokeAsync(Action(grid.Html, "drop", 1)); // [0] panel, [1] region chip
    ...
    public async Task ADragThatStartedNowhere_DoesNothing()
    {
        ...
        await grid.InvokeAsync(Action(grid.Html, "drop"));
    ...
    public async Task DraggingAChipOut_Ungroups()
    {
        ...
        await grid.InvokeAsync(Action(grid.Html, "dragstart")); // [0] the region chip
        await grid.InvokeAsync(Action(grid.Html, "dragend"));   // released on nothing
    ...
    public async Task ADropThenDragEnd_DoesNotAlsoUngroup()
    {
        ...
        await grid.InvokeAsync(Action(grid.Html, "dragstart", 1)); // rep chip
        await grid.InvokeAsync(Action(grid.Html, "drop", 1));      // onto the region chip → [rep, region]
        Assert.Equal(["rep", "region"], reported[^1]);

        var count = reported.Count;
        await grid.InvokeAsync(Action(grid.Html, "dragend"));      // the drag is already spent — no-op
    ...
    public void ThePanelSurvivesTheEmptyState()
    {
        ...
        var html = BsDataGrid.Data(global::System.Array.Empty<Row>())
            ...
            .Grouped(["region"])
            .OnGroupedChange(_ => { })
            .Empty(Div.Id("none")["No rows."]).ToHtml();
