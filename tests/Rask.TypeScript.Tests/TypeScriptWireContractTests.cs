...
// The host element is a wire between two languages: C# writes the attributes, rask-ts.js reads
...
public sealed class TypeScriptWireContractTests
{
    private static string Runtime() =>
        File.ReadAllText(Path.Combine(RepoRoot(), "src", "Rask.TypeScript", "wwwroot", "rask-ts.js"));

    [Theory]
    [InlineData(TypeScriptDefaults.NameAttribute)]
    [InlineData(TypeScriptDefaults.PropsAttribute)]
    [InlineData(TypeScriptDefaults.HydrateAttribute)]
    ...
    public void The_client_runtime_watches_the_props_attribute_for_changes()
    {
        ...
        // look like it worked.
        Assert.Contains($"attributeFilter: [\"{TypeScriptDefaults.PropsAttribute}\"]", Runtime(), StringComparison.Ordinal);
    }
    ...
    {
        Assert.Contains(
            $"HOST_TAG = \"{TypeScriptDefaults.HostTag.ToUpperInvariant()}\"",
            Runtime(),
    ...
    public void The_runtime_script_url_points_at_a_file_this_package_actually_ships()
    {
        ...
        var relative = TypeScriptDefaults.RuntimeScriptUrl["/_content/Rask.TypeScript/".Length..];
        var shipped = Path.Combine(RepoRoot(), "src", "Rask.TypeScript", "wwwroot", relative);

        Assert.True(File.Exists(shipped), $"{TypeScriptDefaults.RuntimeScriptUrl} resolves to {shipped}, which is not there");
    }
