...
public class ComponentFactoryGeneratorTests
{
    ...
    public void IncrementalCacheStability_RerunYieldsCachedCandidates()
    {
        ...
        var firstOutput = first.GeneratedSource("Demo.Generated.g.cs");
        var second = GeneratorDriverFixture.Run(src);
        var secondOutput = second.GeneratedSource("Demo.Generated.g.cs");
    ...
    {
        // Children is now exposed via the `Component this[params Component[]]` indexer, not as
        ...
        Assert.DoesNotContain("params global::System.Collections.Generic.IEnumerable<global::Rask.Core.Component> Children",
            output);
        Assert.DoesNotContain("__c.Children = Children;", output);
    }

    [Fact]
    public void ChildrenProperty_WithInitializer_ExcludedFromFactory()
    {
        var src = """
                  using System.Collections.Generic;
                  using Rask.Core;
        ...
        // Initializer-having props are excluded entirely; the parameterless shape wins.
        Assert.Contains("Card()", output);
    ...
    {
        // A property named "Children" whose type isn't a Component-collection (here, string)
        // must NOT be turned into a params slot — it's a regular factory parameter.
        var src = """
                  using Rask.Core;
                  namespace Demo;
                  public sealed class Card : Component
