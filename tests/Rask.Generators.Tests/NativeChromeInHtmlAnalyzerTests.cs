...
public class NativeChromeInHtmlAnalyzerTests
{
    ...
    [Fact]
    public async Task HtmlInsideNativeScreen_ReportsRask047()
    {
        ...
        var diagnostics = await GetDiagnosticsAsync(src, id: "RASK047");

        var d = Assert.Single(diagnostics);
        Assert.Equal("RASK047", d.Id);
        Assert.Contains("Div", d.GetMessage());
    }

    [Fact]
    public async Task HtmlInsideANativeStack_ReportsRask047()
    {
        var src = """
                  using Rask.Core;
                  using static Rask.Core.Components.Generated;
        ...
        Assert.Equal("RASK047", Assert.Single(diagnostics).Id);
    }
