...
using Rask.Islands.Tasks;

namespace Rask.Islands.Tasks.Tests;
...
// stack trace.
public class IslandBuildPlanTests
{
    [Fact]
    public void A_react_entry_wraps_the_component_with_its_adapter()
    {
        var entry = IslandBuildPlan.EntryModule(
            new IslandEntry { Name = "Chart", Source = "/app/Features/Chart.tsx", Runtime = "react" },
            "/obj/rask-islands/rask");
        ...
        Assert.Contains("import { reactIsland } from '/obj/rask-islands/rask/react'", entry, StringComparison.Ordinal);
        Assert.Contains("export default reactIsland(Component)", entry, StringComparison.Ordinal);
    }
    ...
    {
        var entry = IslandBuildPlan.EntryModule(
            new IslandEntry { Name = "Gauge", Source = "/app/widgets/gauge.ts", Runtime = "lit" },
            "/obj/rask-islands/rask");
        ...
        Assert.Contains("export default litIsland(tag)", entry, StringComparison.Ordinal);
    }
    ...
    public void Windows_paths_become_forward_slashes()
    {
        ...
        var entry = IslandBuildPlan.EntryModule(
            new IslandEntry { Name = "Chart", Source = @"C:\app\src\Chart.tsx", Runtime = "react" },
            @"C:\app\obj\rask");
    ...
    public void A_relative_path_becomes_a_relative_specifier_not_a_bare_one()
    {
        ...
        var entry = IslandBuildPlan.EntryModule(
            new IslandEntry { Name = "Chart", Source = "Features/Chart.tsx", Runtime = "react" },
            "obj/Debug/net10.0/rask-islands/rask");

        Assert.Contains("from './Features/Chart.tsx'", entry, StringComparison.Ordinal);
        Assert.Contains("from './obj/Debug/net10.0/rask-islands/rask/react'", entry, StringComparison.Ordinal);
    }
    ...
    {
        var entry = IslandBuildPlan.EntryModule(
            new IslandEntry { Name = "Chart", Source = "/app/Chart.tsx", Runtime = "react" },
            "/app/obj/rask");
    ...
    {
        var litOnly = Config([new IslandEntry { Name = "Gauge", Source = "/a/g.ts", Runtime = "lit" }]);
        ...
        var withReact = Config([new IslandEntry { Name = "Chart", Source = "/a/c.tsx", Runtime = "react" }]);
    ...
    {
        var config = Config([
            new IslandEntry { Name = "Chart", Source = "/a/c.tsx", Runtime = "react" },
            new IslandEntry { Name = "Gauge", Source = "/a/g.ts", Runtime = "lit" },
        ]);
    ...
    public void The_generated_config_parses_as_javascript()
    {
        ...
        if (node is null)
        {
            Console.WriteLine("IslandBuildPlanTests: no 'node' on PATH — the parse check did NOT run.");
            return;
        }
        ...
        var path = Path.Combine(Path.GetTempPath(), $"rask-islands-{Guid.NewGuid():N}.mjs");
    ...
    private static string Config(IReadOnlyList<IslandEntry> islands) =>
        IslandBuildPlan.ViteConfig(
            islands,
            "/obj/entries",
            "/app/wwwroot/_rask/islands",
            "/app/wwwroot/_rask/islands/manifest.json",
            "/_rask/islands/");
