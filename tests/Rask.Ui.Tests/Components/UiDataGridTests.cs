...
public partial class UiDataGridTests : global::Rask.Core.RaskMarkup
{
    ...
    public void The_column_factory_fixes_the_row_type_so_a_cell_lambda_needs_no_annotation()
    {
        ...
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id)[c => [
    ...
    {
        var html = Ui.DataGrid.Data(Array.Empty<Product>()).RowKey(p => p.Id)[c => [
            c.Field(p => p.Name).Title("Product"),
            c.Field(p => p.Price).Title("Price"),
        ]].ToHtml();
    ...
    {
        var html = Ui.DataGrid.Data(Array.Empty<Product>()).RowKey(p => p.Id).Empty(P["No products yet."])[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
    ...
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).Sort("name")[c => [
            c.Field(p => p.Name).Title("Product").Sortable(true),
    ...
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).Sort("name").SortDescending(true)[c => [
            c.Field(p => p.Name).Title("Product").Sortable(true),
        ]].ToHtml();
    ...
    {
        var grid = (UiDataGrid<Product, int>)Ui.DataGrid.Data(Catalog).RowKey(p => p.Id)[c => [
            c.Field(p => p.Name).Title("Product").Sortable(true),
        ]];
        ...
        var sorted = ((UiDataGrid<Product, int>)Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).Sort("price")[c => [
            c.Field(p => p.Name).Title("Product"),
            c.Field(p => p.Price).Title("Price").Sortable(true),
        ]]).ToHtml();
    ...
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).PageSize(2)[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
    ...
    public void A_pre_sliced_page_is_rendered_as_given_and_counted_by_TotalCount()
    {
        ...
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).PageSize(2).TotalCount(40)[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
    ...
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).PageSize(10)[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
    ...
    public void A_field_may_walk_a_chain_of_members_and_is_named_by_the_last()
    {
        ...
        var html = Ui.DataGrid.Data(orders).RowKey(o => o.Supplier is { } s ? s.Name : "(none)").Sort("name")[c => [
    ...
    public void A_null_partway_along_the_chain_renders_an_empty_cell_rather_than_throwing()
    {
        ...
        var html = Ui.DataGrid.Data(orders).RowKey(o => o.Supplier is { } s ? s.Name : "(none)")[c => [
    ...
    public void A_computed_field_is_refused_and_says_which_step_was_wanted()
    {
        ...
        var error = Assert.Throws<ArgumentException>(() =>
            Ui.DataGrid.Data(Catalog).RowKey(p => p.Id)[c => [
    ...
    {
        var html = Ui.DataGrid.Data(Catalog.AsQueryable()).RowKey(p => p.Id).PageSize(2).Sort("price")[c => [
            c.Field(p => p.Name).Title("Product"),
            c.Field(p => p.Price).Title("Price").Sortable(true),
        ]].ToHtml();
    ...
    public void An_authored_card_replaces_the_stacked_layout_rather_than_joining_it()
    {
        ...
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).Card(p => Span[p.Name])[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
    ...
    public void Selection_appears_only_once_a_selection_is_named()
    {
        ...
        var keyed = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id)[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();

        Assert.DoesNotContain("checkbox", keyed, StringComparison.Ordinal);

        var selectable = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id)
            .Selected([2])[c => [
                c.Field(p => p.Name).Title("Product"),
            ]].ToHtml();
    ...
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).Detail(p => P[p.Category])[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
    ...
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).ColumnChooser(true)[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
    ...
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).GroupPanel(true)[c => [
            c.Field(p => p.Category).Title("Category").Groupable(true),
        ]].ToHtml();
    ...
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).Loading(true)[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
    ...
    public void A_named_grid_keeps_its_name_while_it_is_busy()
    {
        ...
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).Label("Catalog").Loading(true)[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
    ...
    [Theory]
    [InlineData(Ui.Size.Xs, "table-xs")]
    [InlineData(Ui.Size.Lg, "table-lg")]
    public void Every_size_writes_its_own_class(Ui.Size size, string expected)
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).Size(size)[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
    ...
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).Zebra(true).StickyHeader(true).MaxHeight("20rem")[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
    ...
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).RowClass(p => p.Price > 10m ? "text-error" : null)[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
    ...
    public void A_text_column_is_clickable_and_a_custom_cell_is_not()
    {
        ...
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).OnRowClick(_ => { })[c => [
            c.Field(p => p.Name).Title("Product"),
    ...
    public void A_column_shown_from_md_hides_between_sm_and_md_and_nowhere_else()
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id)[c => [
            c.Field(p => p.Name).Title("Product"),
            c.Field(p => p.Category).Title("Category").ShowFrom(Ui.Breakpoint.Md),
        ]].ToHtml();
    ...
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id)[c => [
            c.Field(p => p.Name).Title("Product").ShowFrom(Ui.Breakpoint.Sm),
        ]].ToHtml();
    ...
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).RowTone(p => p.Price > 10m ? Ui.Tone.Error : null)[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
    ...
    {
        var html = Ui.DataGrid.Data(Catalog.Take(2)).RowKey(p => p.Id).PageSize(2).Page(0).TotalCount(6)
    ...
    {
        var html = Ui.DataGrid.Data(Catalog).RowKey(p => p.Id).Toolbar(Span["filters"])[c => [
            c.Field(p => p.Name).Title("Product"),
        ]].ToHtml();
