...
public partial class UiCollapseTests : global::Rask.Core.RaskMarkup
{
    ...
    {
        var html = Ui.Collapse.Title("Advanced")[P["…"]].ToHtml();
    ...
    public void Open_writes_the_open_class() =>
        Assert.Contains("collapse-open", Ui.Collapse.Title("Advanced").Open(true)[P["…"]].ToHtml());

    [Fact]
    public void Closed_writes_collapse_close_rather_than_merely_omitting_collapse_open()
    {
        var html = Ui.Collapse.Title("Advanced").Open(false)[P["…"]].ToHtml();
    ...
    public void The_heading_is_a_button_that_announces_its_state()
    {
        Assert.Contains("aria-expanded=\"true\"",
            Ui.Collapse.Title("Advanced").Open(true)[P["…"]].ToHtml());
        Assert.Contains("aria-expanded=\"false\"",
            Ui.Collapse.Title("Advanced").Open(false)[P["…"]].ToHtml());
    }
    ...
    public void Every_marker_writes_its_own_class(Ui.Marker marker, string expected) =>
        Assert.Contains(expected, Ui.Collapse.Title("Advanced").Marker(marker)[P["…"]].ToHtml());

    [Fact]
    public void No_marker_is_the_default_and_writes_nothing() =>
        Assert.DoesNotContain("collapse-arrow",
            Ui.Collapse.Title("Advanced").Marker(Ui.Marker.None)[P["…"]].ToHtml());

    [Fact]
    public void The_title_and_the_content_are_daisyUIs_two_parts()
    {
        var html = Ui.Collapse.Title("Advanced")[P["hidden thing"]].ToHtml();
