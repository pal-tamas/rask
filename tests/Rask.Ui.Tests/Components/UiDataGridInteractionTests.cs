...
public partial class UiDataGridInteractionTests : global::Rask.Core.RaskMarkup
{
    ...
    public async Task Sorting_returns_to_the_first_page()
    {
        ...
        var page = Page.Render(Ui.DataGrid.Data(Squad).RowKey(r => r.Id).PageSize(2)[c => [
            c.Field(r => r.Name).Title("Name").Sortable(true),
        ]]);
    ...
    {
        var page = Page.Render(Ui.DataGrid.Data(Squad).RowKey(r => r.Id).Detail(r => P[r.Team])[c => [
            c.Field(r => r.Name).Title("Name"),
        ]]);
    ...
    {
        var page = Page.Render(Ui.DataGrid.Data(Squad).RowKey(r => r.Id)[c => [
    ...
    {
        var page = Page.Render(Ui.DataGrid.Data(Squad).RowKey(r => r.Id).GroupPanel(true)[c => [
            c.Field(r => r.Team).Title("Team").Groupable(true),
            c.Field(r => r.Name).Title("Name"),
        ]]);
    ...
    {
        var page = Page.Render(Ui.DataGrid.Data(Squad).RowKey(r => r.Id).ColumnChooser(true)[c => [
            c.Field(r => r.Name).Title("Name"),
    ...
    public async Task An_awaited_source_is_asked_for_the_page_the_grid_wants()
    {
        ...
        var page = Page.Render(Ui.DataGrid.Of<Row, int>()
            .RowKey(r => r.Id)
            .PageSize(2)
            .Source(request =>
            {
