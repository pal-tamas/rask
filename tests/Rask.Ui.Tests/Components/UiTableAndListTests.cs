...
public partial class UiTableAndListTests : global::Rask.Core.RaskMarkup
{
    ...
    {
        var html = Ui.Table[Tbody[Tr[Td["x"]]]].ToHtml();
    ...
    {
        var html = Ui.Table.Class("mb-0").ToHtml();
    ...
    {
        // Nothing here is declared on Ui.Table. Id, Style, Data, Role and Aria all come from Element, which
    ...
    {
        Assert.DoesNotContain("overflow-x-auto", Ui.Table[Tbody].ToHtml(), StringComparison.Ordinal);
    }

    [Fact]
    public void A_scrolling_table_is_a_box_with_the_table_inside()
    {
        var html = Ui.Table.Scroll(true)[Tbody[Tr[Td["x"]]]].ToHtml();
