...
public partial class UiHighlightTests : global::Rask.Core.RaskMarkup
{
    ...
    public void Each_match_is_wrapped_in_mark() =>
        Assert.Equal(
            ...
            Ui.Highlight.Text($"{S}SQLite{E} is {S}fast{E}.").ToHtml());

    [Fact]
    public void Text_without_markers_renders_as_is() =>
        Assert.Equal("<span>plain</span>", Ui.Highlight.Text("plain").ToHtml());

    [Fact]
    public void Stored_markup_is_encoded_inside_and_outside_a_match()
    {
        var html = Ui.Highlight.Text($"<script>x</script> {S}<b>hit</b>{E}").ToHtml();
    ...
    public void An_unclosed_match_marks_the_rest_and_a_stray_end_is_dropped() =>
        Assert.Equal(
            ...
            Ui.Highlight.Text($"a{E} b{S}c d").ToHtml());

    [Fact]
    public void Class_reaches_the_wrapper() =>
        Assert.Equal("<span class=\"text-sm\">x</span>", Ui.Highlight.Text("x").Class("text-sm").ToHtml());

    [Fact]
    public void The_markers_are_the_ones_FullText_emits()
    {
        Assert.Equal(FullText.MatchStart, global::Rask.UiHighlight.MatchStart);
        Assert.Equal(FullText.MatchEnd, global::Rask.UiHighlight.MatchEnd);
    }
}
