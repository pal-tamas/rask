...
public class RaskWasmDiagnosticsTests
{
    ...
    public void Emit_WritesTheMappedLevelCategoryAndMessage_ToTheConsole(int raskLevel, LogLevel expected)
    {
    ...
    [Fact]
    public void AnError_GoesToStandardError_AndAWarning_ToStandardOutput()
    {
        var (output, error) = (new StringWriter(), new StringWriter());
        using var factory = LoggerFactory.Create(b => b.AddProvider(new BrowserConsoleLoggerProvider(output, error)));
    ...
    [Fact]
    public void ADefaultHost_WritesFrameworkDiagnosticsToTheBrowserConsole()
    {
    ...
    [Fact]
    public void AnAppThatRegisteredAProvider_KeepsOnlyItsOwn()
    {
    ...
    [Fact]
    public void BuildingTwice_RegistersTheConsoleProviderOnce()
    {
        var builder = WasmHostBuilder.CreateDefault();
