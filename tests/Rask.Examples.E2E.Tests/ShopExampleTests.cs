...
public sealed class ShopExampleTests(ShopExampleAppFixture app, PlaywrightFixture pw) : IAsyncLifetime
{
    ...
    public async Task The_built_in_dashboard_is_gated_on_the_operator_policy()
    {
        ...
        await Assertions.Expect(_page.Locator("a.ops-nav-link", new() { HasTextString = "Jobs" })).ToBeVisibleAsync();
        await Assertions.Expect(_page.Locator("a.ops-nav-link", new() { HasTextString = "Outbox" })).ToBeVisibleAsync();
    }
    ...
    public async Task The_stored_log_is_searchable_from_the_dashboard()
    {
        ...
        await Assertions.Expect(_page.Locator("a.ops-nav-link", new() { HasTextString = "History" }))
            .ToBeVisibleAsync();
        ...
        await _page.ClickAsync("a.ops-nav-link:has-text('Live')");
