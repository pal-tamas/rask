...
// One GET for a routed page, through the whole ASP.NET pipeline and UseRask — the path a first visit,
// a crawler and a link preview all take, and the one the Server's public-page cache replaces.
//
...
// splicing the document, choosing cache headers — moves no number anywhere else.
//
// In memory rather than over a socket: the handler's cost is what is under measurement, and a socket
// would add the same noise to both sides of every comparison.
//
// The two switches:
//   Static     — whether the page may be served as a document with no session (RenderModes.Static).
//                Off is every app's default, where each GET builds a session the page may never connect.
//   Prerender  — the build switch for the public-page cache, set through its internal test override.
//                Before the cache exists it changes nothing, which is what makes the same four cases a
//                baseline now and the measurement of the cache later.
[MemoryDiagnoser]
public partial class PageRequestBenchmarks
{
    ...
    public void Setup()
    {
        ...
        {
            o.RenderModes.Static = Static;
            o.Prerender = Prerender;
            // An interactive GET leaves a session waiting for a socket that never comes. Released quickly,
            // so a run of thousands of requests measures requests rather than a store filling up.
