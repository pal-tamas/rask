...
{
    [Fact]
    public void SvgTitle_renders_a_title_tag_distinct_from_the_html_title()
    {
    ...
    [Fact]
    public void Setting_every_SvgA_prop_emits_the_expected_attributes() =>
        Assert.Equal(
            "<a href=\"/docs\" target=\"_blank\">link</a>",
            SvgA.Href("/docs").Target("_blank")["link"].ToHtml());

    [Fact]
    public void Setting_every_SvgScript_prop_emits_the_expected_attributes() =>
        Assert.Equal(
            "<script href=\"/a.js\" type=\"text/javascript\"></script>",
            SvgScript.Href("/a.js").Type("text/javascript").ToHtml());

    [Fact]
    public void Setting_every_SvgStyle_prop_emits_the_expected_attributes() =>
        Assert.Equal(
