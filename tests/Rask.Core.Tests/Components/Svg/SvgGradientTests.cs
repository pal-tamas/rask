...
{
    [Fact]
    public void Setting_every_prop_on_a_linear_gradient_emits_the_expected_attributes() =>
        Assert.Equal(
    ...
    [Fact]
    public void Setting_every_prop_on_a_radial_gradient_emits_the_expected_attributes() =>
        Assert.Equal(
    ...
    [Fact]
    public void Setting_every_prop_on_a_stop_emits_the_expected_attributes() =>
        Assert.Equal(
            "<stop offset=\"50%\" stop-color=\"blue\" stop-opacity=\"0.5\"></stop>",
            Stop.Offset("50%").StopColor("blue").StopOpacity("0.5").ToHtml());

    [Fact]
    public void Gradient_with_stop_children_nests_correctly() =>
        Assert.Equal(
    ...
    [Fact]
    public void Setting_every_prop_on_a_pattern_emits_the_expected_attributes() =>
        Assert.Equal(
