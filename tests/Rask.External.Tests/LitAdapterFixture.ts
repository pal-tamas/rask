...
// The props exactly as the generator writes an event prop: a handler id, and `$a` empty because the event never crosses.
const props = (label: string, handler?: string) =>
    JSON.stringify(handler ? {label, '@fx-change': {$h: handler, $a: []}} : {label})
...
island.setAttribute('props', props('Revenue', 'c7:3'))
...
const element = () => island.querySelector('fx-demo') as unknown as {label: string; dispatchEvent(event: unknown): boolean} | null
...
// A re-render that keeps the handler: the runtime revives the same function, so one listener stays one listener.
