...
| `Rask.SQLite`                      | any .NET app using SQLite (server, mobile, trimmed/AOT)            | `services.AddRaskSqlite(cs)` + inject `IRaskSqliteConnectionFactory` |
| `Rask.SQLite.EntityFrameworkCore`  | an EF Core app that wants the pragmas                              | `o.UseRaskSqlite(cs)` on the `DbContextOptionsBuilder`       |
