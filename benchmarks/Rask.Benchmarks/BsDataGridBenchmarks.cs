...
public partial class BsDataGridBenchmarks : global::Rask.Core.RaskMarkup
{
    ...
    // A chain hands back Build<BsDataGrid<Product>>, and RenderAsLiveRoot is a member of the component it
    // builds — so the conversion to the component happens here, at the parameter, once for every benchmark.
    private static string Live(Component grid) => grid.RenderAsLiveRoot();
    ...
    public string Grid100_Plain() =>
        Live(BsDataGrid.Data(Rows).Columns(Columns()).RowKey(p => p.Name));
    ...
    public string Grid100_RowClick() =>
        Live(BsDataGrid.Data(Rows).Columns(Columns()).RowKey(p => p.Name).OnRowClick(_ => { })
            );
    ...
    public string Grid100_RowClass() =>
        Live(BsDataGrid.Data(Rows)
            .Columns(Columns())
            .RowKey(p => p.Name)
            .RowClass(p => p.Stock == 0 ? "table-warning" : null));
    ...
    public string Grid100_Paged20() =>
        Live(BsDataGrid.Data(Rows).Columns(Columns()).RowKey(p => p.Name).PageSize(20));
    ...
    public string Grid100_Selectable() =>
        Live(BsDataGrid.Data(Rows).Columns(Columns()).RowKey(p => p.Name).Selectable(true));
    ...
    public string Grid100_ColumnChooser() =>
        Live(BsDataGrid.Data(Rows).Columns(NamedColumns()).RowKey(p => p.Name).ColumnChooser(true)
            );
    ...
    public string Grid100_HiddenReordered() =>
        Live(BsDataGrid.Data(Rows)
            ...
            .ColumnOrder(["price", "stock", "sku", "category", "name"])
