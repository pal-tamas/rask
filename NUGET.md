...
- **Optional typed Bootstrap** — `Rask.Bootstrap` adds typed Bootstrap 5.3 factories (`BsButton`/`BsCard`/`BsModal`/…), `IFormControl<T>`-bound inputs including the zero-JS `BsSelect`/`BsMultiSelect` comboboxes (full keyboard/listbox a11y, opt-in `Filter` search, `OptionGroup` grouping, `OptionDisabled` per-option disable, and a multiselect `SelectAll` header), a typed `BsIcon`, typed utility classes, and a `BsDataGrid<T>` for list screens (typed columns, sorting, paging, footer totals, master-detail), with interactive components driven by the live runtime — no JavaScript. See [docs/bootstrap.md](https://github.com/pal-tamas/rask/blob/main/docs/bootstrap.md).
