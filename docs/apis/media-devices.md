...
Capture needs transient activation + a secure (HTTPS) context, so the imperative `IMediaDevices` service is WASM-only. On the **Server** host, use the declarative **`Trigger.MediaCapture`** component — point its `For:` at a `<video>`'s `ElementRef`, set `Audio` / `Video` / `FacingMode`, and its click starts the stream and attaches it to that element inside the gesture, posting `"granted"` / `"denied"` to `OnResult`.
...
> 🟡 On the Server host, reachable declaratively via `Trigger.MediaCapture` (a click-gesture component), not as an injected service.
