...
| **The door out of one box** | ✅ | [`databases.md`](databases.md) — `rask new --database postgres` wires PostgreSQL via `Rask.Postgres` (production session settings + retry), and deploy/`rask db` follow. Jobs, mail and the outbox **lease** the work they claim, so several instances is safe; a lease bounds, but does not eliminate, a duplicate side effect. |
