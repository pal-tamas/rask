# TypeScript components — a `.tsx` file as a Rask component

A **TypeScript component** is an ordinary Rask component whose markup is produced by a front-end
...
runtime on WASM. A TypeScript component never opens a channel of its own, so a callback inherits sequence
...
```csharp
Chart.Series(_points).Hydration(TypeScriptHydration.Visible)
```
...
A TypeScript component can wrap Rask-rendered content, so replacing a component in the middle of a tree does not
strand its descendants:

```csharp
Panel.Heading("Sales")[
    TypeScriptSlot.Named("footer")[ BsButton["Save"] ],
...
It is called `TypeScriptSlot`, not `Slot`, because `Slot` is already the HTML `<slot>` element in
...
| Property | Default | |
|---|---|---|
| `RaskTypeScriptBuild` | `true` | `false` skips node entirely. They still render their host elements. |
| `RaskTypeScriptOutputDir` | `wwwroot/_rask/ts` | Under `wwwroot` so the SDK publishes it with no publish target of its own. |
| `RaskTypeScriptPublicBase` | `/_rask/ts/` | The URL prefix the manifest gives each chunk. |
...
```xml
<ItemGroup>
  <RaskTypeScript Include="widgets/gauge.ts" Runtime="lit"/>
</ItemGroup>
```
