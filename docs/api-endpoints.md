...
generator running there would see no controllers at all.

What crosses instead is the **file** the server's own generator wrote. `EmitCompilerGeneratedFiles`
puts it on disk, and the browser app's generated project compiles that exact file — so the two halves
cannot disagree about the client, because there is only one of it. The client runtime
(`Rask.Api.Client`) is added to the browser app automatically when that file exists; you do not declare
it twice.
...
> **The shapes that cross the wire live in `Shared/`.** A request or response type declared anywhere else
> on the server side is invisible to the browser app, so the generated client returns a type it cannot
> compile — and you meet it as `CS0246` inside generated code you never wrote. Same rule as a CQRS
> message record: the **handler** is server-only, the **shape** is shared. In practice: `Post`, `NewPost`
> and friends in `Shared/`, `PostsController` outside `Client/` and `Shared/`.
