...
| `--database <sqlite\|postgres>` | Which database `--data` wires. Defaults to `sqlite` — one file, no server. `postgres` uses [`Rask.Postgres`](databases.md) instead, and drops the file-only batteries (Litestream, snapshots, the deploy volume). The `server` template only. |
