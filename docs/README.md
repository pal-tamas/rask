...
| [Subscriptions](subscriptions.md) | `QueryClient.Subscribe<T>()`: a published CQRS notification re-renders every subscribed page — scoped with `[For<T>]` and a watch policy, over server-sent events from WebAssembly, across servers through Redis. |
