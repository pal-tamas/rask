...
| [Islands](islands.md) | A `.tsx` or Lit file as an *ordinary Rask component*: mark it `[Island]`, drop the front-end file beside it, and place it anywhere the chain goes — a leaf, a subtree, or a whole route. Props are declared in C# and serialized without reflection, callbacks re-enter C# over the channel every DOM handler already uses, and the live diff treats the island's subtree as opaque because its own renderer owns it. |
