...
| [Dashboard](dashboard.md) | A built-in operator dashboard at `/_ops` via `AddRaskDashboard<Ctx>()` (standalone `Rask.Dashboard`) — queue depth and dead letters for the outbox/jobs/mail, cache contents, a log tail, SQLite pragmas; fail-closed behind an authorization policy. |
