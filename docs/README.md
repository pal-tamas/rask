...
| [UI kit](ui-kit.md) | The components the framework's own surfaces are drawn with (standalone `Rask.Ui`) — shell, tab bar, breadcrumb switcher, buttons, metric rows, detail lists, sheets and toasts. Mobile-first, Tailwind-only, ships no JavaScript, and brings its own compiled stylesheet so a consuming app configures nothing. |
