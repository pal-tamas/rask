...
| [TypeScript front ends](spa.md) | A TypeScript SPA with a typed connection to your C#: `rask new --template react`, TypeScript generated from the message records, TanStack Query, and `UseRaskSpa()` (standalone `Rask.Spa.Hosting`). React, Vue or Angular — the framework is yours, the language is not. |
