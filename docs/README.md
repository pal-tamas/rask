...
| [Browser APIs](browser-apis.md) | The map of all 48 typed Web-API wrappers — shared vs WASM-only, one-shot vs subscription, the inject-from-ctor and push/`[JSInvokable]` patterns. |
| [Capability matrix](browser-capabilities.md) | Where each of the 48 APIs works (Web / PWA / Native) and which have a native iOS/Android backend — links to a reference page per API under [`apis/`](apis/). |
