...
| Guide | What it covers |
|-------|----------------|
| [Rask.Data](data.md) | Declare a `Model<TId>` and nothing else: reads off the type (`Product.Where(…)`), generated form models (`ProductModel`), writes as plain EF Core through a context, audit stamps, soft delete, optimistic concurrency and domain events. |
