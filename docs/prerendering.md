...
a server's first response uses, so **a page whose `OnMount` loads build-time data writes the
...
- **`.br` / `.gz` siblings are regenerated** from the new page. Left stale, they still hold the SHELL —
  ...
  S3 behind a CDN, Rask's own `MapRaskSpa`) serves the spinner to every visitor and every
...
- **`/docs/guides/lifecycle` is the one page that does not settle.** It embeds two demos whose
  `OnMount` runs an unbounded poll loop, so the hook's task never completes and the wave loop
