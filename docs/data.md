...
component; see [live queries](query.md#staying-fresh). Rails says this with
`after_update_commit { broadcast_replace_to … }`; the difference is that nothing is published unless a page is
listening, and that the page's own query decides who may see the refresh.
