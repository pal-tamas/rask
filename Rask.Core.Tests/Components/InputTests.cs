...
public class InputTests
{
    ...
    public void Render_OnInputOutsideLiveContext_OmitsHandlerAttribute()
    {
        Assert.Equal(
            "<input />",
            new Input { OnInput = _ => { } }.ToHtml());
    }

    [Fact]
    public void Render_OnInputAndOnChangeInsideLiveContext_EmitSequentialIds()
    {
        var view = new StubComponent(() => new Input { OnInput = _ => { }, OnChange = _ => { } });
        Assert.Equal(
    ...
    {
        var view = new StubComponent(() => new Input { OnInputAsync = async _ => { await Task.Yield(); }, OnChangeAsync = async _ => { await Task.Yield(); } });
        Assert.Equal(
            "<input data-rask-on-input=\"h0\" data-rask-on-change=\"h1\" />",
            view.RenderAsLiveRoot());
    }
