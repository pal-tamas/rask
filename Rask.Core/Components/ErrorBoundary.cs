...
public sealed class ErrorBoundary : Component
{
    ...
    private Func<Exception, Action, Child>? _fallback;
    private bool _hasResetKeys;
    private object?[]? _resetKeys;

    internal Exception? Error { get; private set; }

    // Boundary state (_error) lives outside the framework's prop/state diff, so the cached
    ...
    internal void SetProps(IEnumerable<Child>? children, Func<Exception, Action, Child>? fallback, IReadOnlyList<object?>? resetKeys)
    {
        _children = children;
        _fallback = fallback;

        // Snapshot to a fresh array so a caller mutating the source list between renders
        // can't poison our equality check.
        var snapshot = resetKeys is null ? null : resetKeys.ToArray();

        // First call seeds without resetting; subsequent calls clear the error when any
        // element has changed by Equals (React's useEffect-deps semantics).
        if (_hasResetKeys && KeysChanged(_resetKeys, snapshot))
        {
            Error = null;
        }
    ...
    protected override Component Render()
    {
        ...
        if (_fallback is not null)
        {
            ...
            return new Fragment(_fallback(Error, Recover));
        }
