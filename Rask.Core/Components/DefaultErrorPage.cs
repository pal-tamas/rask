...
public sealed class DefaultErrorPage : Component
{
    ...
    protected override Component Render()
    {
        ...
        var children = new List<Child>
        {
            ...
            Components.P(Style: "margin:0 0 0.5rem;color:#4b5563;font-family:ui-monospace,Menlo,Consolas,monospace;font-size:0.9rem;")[typeName],
            Components.Pre(
                Style:
                "margin:0;padding:0.75rem;background:#fbe9e9;border-radius:0.375rem;white-space:pre-wrap;font-size:0.9rem;color:#7f1d1d;")[_error.Message]
        };
        ...
        if (showStack && !string.IsNullOrEmpty(_error.StackTrace))
        {
            children.Add(Components.Pre(
                Style:
                "margin:1rem 0 0;padding:0.75rem;background:#f3f4f6;border-radius:0.375rem;white-space:pre-wrap;font-size:0.78rem;color:#4b5563;overflow:auto;max-height:300px;")[_error.StackTrace]);
        }

        return Components.Div(
            Class: "rask-error-boundary",
            Style:
            "max-width:720px;margin:4rem auto;padding:1.5rem;font-family:system-ui,sans-serif;color:#1f2937;border:1px solid #f5c2c0;background:#fff5f5;border-radius:0.5rem;")[children];
    }
