...
[Route(""), ParentRoute(typeof(ShowcaseLayout))]
        ...
        Fragment(
            ...
            Components.CodeSample(
                Source: """
                    ...
                    """,
                Title: "The minimal page",
                Notes: "Static factories from Rask.Core.Tags build a tree. Strings convert implicitly to Child. Component.ToHtml() produces the final HTML.",
                Result: Fragment(
            ...
            [
                FeatureCard("bi-code-slash", "DSL", "Every HTML element as a strongly-typed factory. Universal Id/Class/Style/Data on every tag.", "/tags"),
                FeatureCard("bi-boxes", "Components", "Sealed classes with Render(). Source-generated factories with required and optional params, DI through the constructor.", "/components"),
                FeatureCard("bi-signpost-2", "Routing", "[Route], [ParentRoute], [RouteParam], [QueryParam]. Nested layouts via Outlet().", "/routing"),
                FeatureCard("bi-arrow-repeat", "Lifecycle", "OnInitialized, OnParametersSet, OnAfterRender — sync and async, with auto re-render after each await.", "/lifecycle"),
                FeatureCard("bi-palette", "Scoped CSS", "Co-locate styles on the component. Rask hashes the type name and rewrites selectors so two components can share .box.", "/scoped-css"),
                FeatureCard("bi-cloud-arrow-down", "HttpClient + DI", "Standard ServiceCollection. Inject HttpClient via the primary constructor and fetch in OnInitializedAsync.", "/http"),
            ]),
