...
public sealed partial class App : Component
{
    ...
    // per sample. The palette lives in Styles/app.css, compiled with it — user <head>
    // contributions splice in before the scoped-css links the framework appends.
    ...
    [
        Nav.Class("flex items-center border-b border-slate-800 bg-slate-900 text-slate-100 shadow-sm")[
            Div.Class("mx-auto w-full max-w-6xl px-4")[
                NavLink.Href(Features.Routes.HomePage()).Class("app-brand font-bold")["Rask · cookie + WASM auth"],
                A
