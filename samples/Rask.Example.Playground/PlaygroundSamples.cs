...
///     project does — that's what lets the generator's <c>global using static Demo.Generated;</c> bring a
///     user component's own factory into scope), and uses only types in the shipped <c>_framework</c>
///     (BCL + Rask.Core). Note the explicit <c>using</c>s: the in-browser compile has no MSBuild implicit
///     usings, only the generator's <c>global using static …Generated;</c>, so a snippet must bring in
