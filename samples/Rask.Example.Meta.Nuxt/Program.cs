...
builder.Services.AddSingleton<Rask.Example.Meta.Nuxt.Features.Hello.VisitCounter>();
...
builder.Services.AddRaskMeta(o =>
{
    ...
    // own address in RASK_BASE_URL, and the page reaches for that when it is rendering rather than
    // hydrating. Taken from the URL this host was told to listen on, which is what `rask dev`, the
    // launch profile and the E2E fixture all set.
    //
    // Configured rather than derived from an incoming request, deliberately: a header an attacker can
    // influence, turned into the destination of a request carrying the visitor's cookie, is a confused
    // deputy.
