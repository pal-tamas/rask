...
|---------|------------|-----|
| [`Rask.Example.Shop`](Rask.Example.Shop) | **Every OPF battery in one app** — data, CQRS, outbox, jobs, mail, cache, production SQLite, snapshots, Litestream, Web Push, auth, Docker. Generated by the CLI (`rask new --all-batteries` + `rask generate`), not hand-written. Its `/ops` page shows every pillar's state in the one SQLite file. | `dotnet run --project samples/Rask.Example.Shop` |
