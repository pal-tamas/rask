...
public sealed class UploadPage : Component
{
    ...
    [
        PageHeader.Render(
            "File upload",
            "Input<string>(InputType.File, OnFiles: …) wires a file picker to a typed handler. RaskFile carries the metadata; OpenReadStream gives you a Stream for the bytes — over multipart POST on the server, via JS chunked reads on WASM."),
