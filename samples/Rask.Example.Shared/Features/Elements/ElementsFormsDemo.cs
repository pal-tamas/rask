...
public sealed partial class ElementsFormsDemo : Component
{
    ...
    protected override Component? Render() => Form.Model(_fields).Class("flex flex-col gap-3")[
        Fieldset.Class("border rounded p-3")[
            ...
            Div.Class("mb-2")[
                Label.For("nm").Class($"{Ui.Label} text-sm mb-1")["Name"],
            ...
            Div.Class("mb-2")[
                Label.For("fruit").Class($"{Ui.Label} text-sm mb-1")["Favourite"],
                Select.Value<string>(null).Id("fruit").Name("fruit").Class(Ui.Select)[
            ...
            Div.Class("mb-0")[
                Label.For("bio").Class($"{Ui.Label} text-sm mb-1")["Bio"],
                Textarea.Value<string>(null).Id("bio").Class(Ui.Input).Placeholder("About you…")
            ]
        ],
        Div.Class("grid grid-cols-12 gap-4 items-center")[
            Div.Class("col-auto")[
                Label.Class($"{Ui.Label} text-sm mb-1")["Progress"], Br,
                Progress.Value(0.6).Max(1.0)
            ],
            Div.Class("col-auto")[
                Label.Class($"{Ui.Label} text-sm mb-1")["Meter"], Br,
                Meter.Value(0.8).Min(0).Max(1).Low(0.2).High(0.9).Optimum(1)
            ],
            Div.Class("col-auto")[
                Label.Class($"{Ui.Label} text-sm mb-1")["Output"], Br,
