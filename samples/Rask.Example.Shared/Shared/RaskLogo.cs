...
internal sealed partial class RaskLogo : Component
{
    public required double Size { get; set; }

    // MUST be unique per call site on a given page — the navbar brand and the home hero both render the
    // mark on "/", and two <linearGradient> elements sharing an id would collide (the second fill
    // silently resolves to the first definition).
    public required string GradientId { get; set; }
