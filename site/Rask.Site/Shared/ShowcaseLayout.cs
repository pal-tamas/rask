    ...
    : Component
{
    ...
    [
        ...
        // in global.css, which outranks every layered utility on the page, so it beat the `bg-base-100`
        // written right here. The bar rendered near-black while its own text stayed `text-base-content`
        ...
        // The glass survives — `bg-base-100/85` + a backdrop blur — but drawn from the palette, and it now
        // matches the landing page's header exactly, which is the same three utilities.
        Nav.Class(
            "app-navbar navbar sticky top-0 z-50 flex-nowrap gap-3 border-b border-base-300 "
            + "bg-base-100/85 px-3 pt-[calc(0.5rem_+_env(safe-area-inset-top))] text-base-content "
        ...
        Div.Class("flex items-start app-shell")[
            ...
            Aside
                ...
                .Class(_drawerOpen
                    ? "side-nav flex fixed inset-0 z-40 w-full bg-base-100 p-4 "
