        ...
        Div.Class($"{Tw.Card} shadow-sm border-0")[
            Div.Class(Tw.CardBody)[
                Div.Class("flex gap-2 flex-wrap items-center mb-2")[
                    Button.Class(Tw.BtnOutlinePrimary).Id("crypto-uuid").OnClick(Uuid)["Random UUID"],
                    Button.Class(Tw.BtnOutlinePrimary).Id("crypto-bytes").OnClick(Bytes)[
                        "Random bytes"]
                ],
                ...
                Input
                    ...
                    .Class($"{Tw.Input} mb-2")
                    .OnInput(v => _text = v),
                Button.Class($"{Tw.BtnPrimary} mb-2").Id("crypto-hash").OnClick(Hash)["SHA-256"],
