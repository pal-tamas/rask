...
// The repo's user guides (docs/*.md) rendered on-site, grouped as cards. Each card links to
// /docs/guides/{slug}, where GuidePage renders the markdown with Markdig.
//
// The index of the showcase, at ShowcaseLayout's root — which is /docs. Same URL it has always had:
// this used to be a separate app published under /docs/, and rooting the layout there rather than at /
// is what let the landing page take the site root without changing a single published link.
[Route("")]
...
public sealed partial class GuidesIndexPage : Component
{
    ...
    private static Component Prompt() => Span.Class("select-none text-ui-brand-ink")["$"];
}
