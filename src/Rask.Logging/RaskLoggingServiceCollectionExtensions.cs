...
public static class RaskLoggingServiceCollectionExtensions
{
        ...
        Action<RaskLoggingOptions>? configure = null)
    {
        ...
        return AddStore(services, configure, sp => new SqliteLogStore(
            connectionString,
    ...
    /// not captured back into it; your own SQL still is.
    /// </para>
    /// <para>
    /// On SQLite prefer <see cref="AddRaskLogging(IServiceCollection, string, Action{RaskLoggingOptions}?)"/>: a
    /// file of its own keeps a machine-rate writer off the single write lock your requests share.
    /// <see cref="RaskLoggingOptions.Pragmas"/> and <see cref="RaskLoggingOptions.BusyRetry"/> do not apply here.
    /// Register <typeparamref name="TContext"/> as an <see cref="IDbContextFactory{TContext}"/>; a model that never
    /// mapped the table fails the boot with the line to add. Idempotent.
    /// </para>
    /// </summary>
        ...
        where TContext : DbContext
    {
        ...
        // error every second. It reads the MODEL, never the database — see BatteryModelCheck.
        services.AddHostedService<LogsModelCheck<TContext>>();

        return AddStore(services, configure, sp => new DbContextLogStore<TContext>(
        ...
        Func<IServiceProvider, ILogs> store)
    {
        ...
        // The first registration wins: an app that calls both overloads keeps the store it registered first.
