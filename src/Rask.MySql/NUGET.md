...
`UseRaskMySql(sp)` reads the connection string from `Rask:ConnectionStrings:App` — a missing one is an error naming
that key, never a guessed server — and every default below from the `Rask:MySql` section:
...
// appsettings.json
{
  "Rask": {
    "ConnectionStrings": {
      "App": "Server=db;Database=app;User ID=app"
    },
    "MySql": {
...
In production set them as environment variables (`Rask__ConnectionStrings__App`, `Rask__MySql__LockTimeout`). A
configure delegate runs after the section and wins:

```csharp
o.UseRaskMySql(sp, m => m.Retry.Enabled = false);
```
