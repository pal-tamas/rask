...
{
    /// <summary>
    /// Configures the context to use MySQL with the <c>Rask:ConnectionStrings:App</c> connection string, applies the
    /// <see cref="MySqlOptions"/> session settings on every connection open — defaults, then the <c>Rask:MySql</c>
    /// configuration section, then <paramref name="configure"/> — retries transient failures, and stores every
    /// <see cref="DateTimeOffset"/> as its UTC instant so it keeps its fractional seconds. Swapping an existing
    /// <c>UseMySQL(cs)</c> for <c>UseRaskMySql(sp)</c> needs a migration, because those columns become
    /// <c>datetime(6)</c>.
    /// <code>
    /// builder.Services.AddDbContextFactory&lt;AppDbContext&gt;((sp, o) =&gt; o.UseRaskMySql(sp));
    /// </code>
    /// </summary>
    ...
    /// <param name="configure">Overrides for the production defaults, applied after the <c>Rask:MySql</c> section.</param>
        ...
        Action<MySqlOptions>? configure = null)
    {
        ...
        var options = RaskOptionsRegistration.BindNow<MySqlOptions>(
            services, "Rask:MySql", static (section, o) => section.Bind(o), configure);
        ...
        catch (Exception ex) when (ex is ArgumentException or InvalidOperationException)
        {
            ...
            throw new OptionsValidationException(
                Microsoft.Extensions.Options.Options.DefaultName, typeof(MySqlOptions), [$"Rask:MySql: {ex.Message}"]);
        }
    ...
    /// <see cref="UseRaskMySql(DbContextOptionsBuilder, IServiceProvider, Action{MySqlOptions}?)"/>, so
    /// <c>new DbContextOptionsBuilder&lt;TContext&gt;().UseRaskMySql(services).Options</c> keeps its
    ...
    {
        UseRaskMySql((DbContextOptionsBuilder)optionsBuilder, services, configure);
        return optionsBuilder;
    }
}
