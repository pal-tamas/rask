...
/// </remarks>
public sealed class WriteTypeScriptBuildInputsTask : Task
{
    ...
    public override bool Execute()
    {
        ...
        var islands = new List<TypeScriptEntry>();
        ...
        foreach (var item in Islands)
        {
            ...
            islands.Add(new TypeScriptEntry
            {
                Name = name,
                Source = source,
        ...
        foreach (var island in islands)
        {
            ...
            written += TypeScriptBuildPlan.WriteIfDifferent(entry, TypeScriptBuildPlan.EntryModule(island, AdapterDirectory))
                ? 1
                : 0;
        }

        ConfigPath = Path.Combine(IntermediateDirectory, "vite.islands.config.mjs");
        written += TypeScriptBuildPlan.WriteIfDifferent(
            ConfigPath,
            TypeScriptBuildPlan.ViteConfig(islands, entryDirectory, OutputDirectory, ManifestPath, PublicBase))
            ? 1
            : 0;
