...
///     </para>
///     <para>
///         <b>RASK047</b> — the mirror image: an HTML element nested inside a <c>NativeScreen</c> subtree. A
...
public sealed class NativeChromeInHtmlAnalyzer : DiagnosticAnalyzer
{
    ...
    public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics { get; } =
        ImmutableArray.Create(Rask032, Rask047);
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        context.RegisterCompilationStartAction(static start =>
        {
            ...
            start.RegisterSyntaxNodeAction(
                ctx => AnalyzeChildren(ctx, nativeComponent, nativeView, element),
        ...
        INamedTypeSymbol? element)
    {
        ...
        var receiver = Resolve(context, node.Expression);
            ...
            if (receiverIsNativeView)
            {
                if (element is not null && DerivesFrom(type, element))
                {
                    context.ReportDiagnostic(
                        Diagnostic.Create(Rask047, arg.Expression.GetLocation(), type.Name));
                }
    ...
    /// </summary>
    /// <remarks>
    ///     The unwrap itself is <see cref="BuilderEntry.ChainedComponent" />'s, shared with the other
    ///     analyzers that went quiet when the chain's receiver changed. Keeping a private copy here would be
    ///     a second mechanism free to drift from the first.
    /// </remarks>
