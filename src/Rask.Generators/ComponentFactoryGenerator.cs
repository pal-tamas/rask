...
public sealed class ComponentFactoryGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        // An ABSTRACT component is deliberately not a candidate — it has no factory and no entry of its
        // own, because nothing can construct it. But it is still a component, and the builder surface is
        // only reachable from inside one: an abstract base that composes other components (BsBlock,
        // BsFormControl<T>, PollingPanel) could name no entry at all and fell back to the factory. So it
        // is collected separately, purely as an injection HOST — never as something to emit an entry for.
        var abstractHosts = context.SyntaxProvider
            .CreateSyntaxProvider(
                static (node, _) => node is ClassDeclarationSyntax { BaseList.Types.Count: > 0 } c &&
                                    c.Modifiers.Any(m => m.IsKind(SyntaxKind.AbstractKeyword)),
                static (ctx, _) => GetAbstractHost(ctx))
    ...
    private static void EmitConsumerEntries(
        ...
        ImmutableArray<EntryHostDecl> abstractHosts)
    {
        if (!enabled || host.DeclaresComponent || candidates.IsDefaultOrEmpty)
        {
            return;
        ...
        foreach (var host2 in EntryHostDecls(candidates, abstractHosts))
        {
            if (host2.IsNested)
            {
    ...
    private static List<EntryHostDecl> EntryHostDecls(
        ImmutableArray<Candidate> candidates, ImmutableArray<EntryHostDecl> abstractHosts)
    {
        var all = new List<EntryHostDecl>(candidates.Length + abstractHosts.Length);
        ...
        if (!abstractHosts.IsDefaultOrEmpty)
        {
            all.AddRange(abstractHosts);
        }
    ...
    // An abstract component read purely as an injection host. Deliberately none of Candidate's
    // construction facts (constructors, props, form-control shape): nothing here is ever built.
    private static EntryHostDecl? GetAbstractHost(GeneratorSyntaxContext ctx)
    {
        ...
        if (!InheritsFromComponent(symbol) || IsInRaskCoreNamespace(symbol) || HasSkipFactoryAttribute(symbol))
        {
            return null;
        }
