...
public sealed class ComponentFactoryGenerator : IIncrementalGenerator
{
    ...
    private static void Emit(SourceProductionContext spc, ImmutableArray<Candidate> candidates, bool emitNavigation)
    {
        ...
        foreach (var group in byNamespace)
        {
            ...
            sb.AppendLine("// <auto-generated />");
            sb.AppendLine("#nullable enable");
            // A factory documents the properties that carry a summary and leaves the rest bare, so
            // the undocumented ones would each be CS1573 in any consumer that emits a doc file —
            // which is every packable project here. Partial documentation is the normal state (an
            // element factory carries ~50 universal event props); it must not fail a build.
