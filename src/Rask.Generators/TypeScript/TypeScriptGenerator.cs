...
namespace Rask.Generators.TypeScriptComponents;
...
/// <remarks>
///     <para>
///         Almost everything that makes a component TypeScript-backed is hand-written on
///         <c>TypeScriptComponent</c> itself — the host element, the opaque-subtree boundary, the slot
...
{
    private const string ReactBaseName = "Rask.TypeScript.ReactComponent";
    private const string LitBaseName = "Rask.TypeScript.LitComponent";
    private const string SkipFactoryName = "Rask.Core.SkipFactoryAttribute";

    // RASK057 ("declares its own Render") is retired. TypeScriptComponent seals Render(), so writing
    ...
    private static readonly DiagnosticDescriptor Rask055 = new(
        "RASK055",
        "TypeScript component must be partial",
        "'{0}' must be declared 'partial' — its name, module and props writer are generated into the same class",
        DiagnosticHelp.Category,
        DiagnosticSeverity.Error,
        true,
        description: "A TypeScript component is completed by a second part of the class. Without 'partial' there is "
                     + "nowhere to put it, so the three members TypeScriptComponent declares abstract are never "
    ...
    private static readonly DiagnosticDescriptor Rask056 = new(
        "RASK056",
        "TypeScript component prop has no wire encoding",
    ...
    private static readonly DiagnosticDescriptor Rask058 = new(
        "RASK058",
        "TypeScript component name collision",
    ...
    private static void Execute(SourceProductionContext spc, Compilation compilation)
    {
        ...
        {
            // The app does not reference Rask.TypeScript. Nothing to do, and not a problem.
            return;
        }
        ...
        {
            spc.AddSource($"{island.Fqn.Replace("global::", string.Empty)}.TypeScript.g.cs",
    ...
    private static string Emit(ComponentModel island)
    {
        ...
        foreach (var handler in island.Handlers)
        {
            ...
            body.AppendLine("                writer.WriteString(\"$h\", "
                            + $"global::Rask.TypeScript.TypeScriptHandlers.Register(this, this.{handler.ClrName}));");
        ...
        // hand-written on TypeScriptComponent, where it can be read.
