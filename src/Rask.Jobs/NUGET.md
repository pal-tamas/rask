...
{
    public Task Handle(SendWelcomeEmail cmd) => email.SendWelcomeAsync(cmd.UserId, Current.Cancellation);
}
...
```csharp
// enqueue from anywhere — a handler, a render, a request, another job:
await Jobs.Enqueue(new SendWelcomeEmail(user.Id));
await Jobs.Enqueue(new SendReminder(order.Id)).In(24.Hours);
```
