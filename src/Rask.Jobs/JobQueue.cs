...
/// <summary>
/// The default <see cref="IJob"/>: writes one <see cref="Job"/> row through the app's
...
public sealed class JobQueue<TContext>(IDbContextFactory<TContext> contextFactory, TimeProvider timeProvider) : IJob
    where TContext : DbContext
{
    /// <inheritdoc/>
    public Task EnqueueAsync(IBackgroundJob job, CancellationToken cancellationToken = default) =>
        WriteAsync(job, timeProvider.GetUtcNow().UtcDateTime, cancellationToken);

    /// <inheritdoc/>
    public Task ScheduleAsync(IBackgroundJob job, TimeSpan delay, CancellationToken cancellationToken = default) =>
        WriteAsync(job, timeProvider.GetUtcNow().UtcDateTime + delay, cancellationToken);

    /// <inheritdoc/>
    public Task ScheduleAsync(IBackgroundJob job, DateTimeOffset runAt, CancellationToken cancellationToken = default) =>
        WriteAsync(job, runAt.UtcDateTime, cancellationToken);

    private async Task WriteAsync(IBackgroundJob job, DateTime runAt, CancellationToken cancellationToken)
    {
