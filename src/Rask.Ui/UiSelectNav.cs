...
// cursor is a FLAT index, and grouping only reorders the flat list rather than nesting it, so an
// arrow key moves to the next rendered option and not to the next option in some parallel structure:
// the cursor follows the eye. Every mover takes a `disabled` predicate, so an unselectable option is
// skipped over rather than landed on.
//
