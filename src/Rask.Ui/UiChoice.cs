...
internal static class UiChoice
{
    ...
    internal static string ContainerClass(UiChoiceLayout layout) => layout switch
    {
        UiChoiceLayout.Cards => "grid gap-2 sm:grid-cols-2",
        UiChoiceLayout.Pills or UiChoiceLayout.Buttons => "flex flex-wrap gap-2",
        UiChoiceLayout.Segmented => "join",
    ...
    internal static string ChoiceClass(UiChoiceLayout layout) => layout switch
    {
        UiChoiceLayout.Cards =>
            "flex cursor-pointer items-start gap-3 rounded-box border border-base-300 p-3 "
            + "has-[:checked]:border-primary has-[:checked]:bg-primary/5 "
            + "has-[:disabled]:cursor-not-allowed has-[:disabled]:opacity-50",
        UiChoiceLayout.Pills =>
    ...
    internal static bool ShowsBox(UiChoiceLayout layout) =>
        layout is UiChoiceLayout.List or UiChoiceLayout.Cards;
