...
A TanStack Start front end on an ASP.NET host: one project, one container, one port.
...
| `Company.RaskServer/client/` | The TanStack Start app, as `@tanstack/cli` scaffolds it, plus the config Rask adjusts. |
...
Two processes. The browser talks to TanStack Start's own dev server on
