...
- **List screens (Rask.Bootstrap):** `BsDataGrid(Data: rows, Columns: [...])` with typed `BsColumn<T>` columns bound straight to the row type — `Sortable`/`SortKey` for click-to-sort headers, `PageSize` for a pager, `Footer` for a column total over every row, `Template` for a custom cell, `Empty` for the no-rows state, and `ExpandedContent` (+`RowKey`) for master-detail. Zero JS. Don't hand-roll a `<table>` for a list screen.
- **CQRS (`Rask.Cqrs`):** opt-in, source-generated mediator that is **reflection-free, so the WASM client publishes clean under the trimmer**. `dotnet add package Rask.Cqrs`, define `IQuery<T>`/`ICommand`/`ICommand<T>`/`INotification` messages + their handlers, call `AddRaskCqrs()`, then inject `IDispatcher` and dispatch — the response type is inferred. Pipeline behaviors are the decorator hook (`AddOpenBehavior`). See docs/cqrs.md.
- **Full WASM AOT (optional):** the WASM client publishes on the Mono interpreter by default. To AOT-compile IL→WASM, `dotnet workload install wasm-tools` then publish the client with `-p:RaskWasmAot=true`. Custom `IParsable<T>` **form-field** types must be registered once at startup with `RaskBinding.RegisterParsable<T>()` (route/query param types are auto-registered); custom `InvokeAsync<T>` result types need a `JsonSerializerContext`. See docs/aot.md.
...
- **Deploy (`--docker`):** scaffolded with `dotnet new rask-wasm-hosted --docker` — a multi-stage `Dockerfile` (+ `.dockerignore`) that installs `wasm-tools`, builds both projects, and runs the Host on `aspnet:10.0` (non-root, port 8080). `docker build -t myapp . && docker run -p 8080:8080 myapp`. Terminate TLS at your proxy and forward to 8080. See docs/deployment.md.
