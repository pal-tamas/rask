...
- **CQRS (`Rask.Cqrs`):** opt-in, source-generated mediator. `dotnet add package Rask.Cqrs`, define `IQuery<T>`/`ICommand`/`ICommand<T>`/`INotification` messages + their handlers, call `builder.Services.AddRaskCqrs()`, then inject `IDispatcher` (or read-only `IQueryDispatcher` / write-only `ICommandDispatcher`) and dispatch — the response type is inferred. Reflection-free (trim/AOT-safe); pipeline behaviors are the decorator hook (`AddOpenBehavior`). See docs/cqrs.md.
