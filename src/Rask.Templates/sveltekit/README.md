...
A SvelteKit front end on an ASP.NET host: one project, one container, one port.
...
| `Company.RaskServer/client/` | The SvelteKit app, as `sv` scaffolds it, plus the config Rask adjusts. |
...
Two processes. The browser talks to SvelteKit's own dev server on
