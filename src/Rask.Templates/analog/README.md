...
A Analog front end on an ASP.NET host: one project, one container, one port.
...
| `Company.RaskServer/client/` | The Analog app, as `create-analog` scaffolds it, plus the config Rask adjusts. |
| `Company.RaskServer/client/src/rask/` | Generated on every build: your C# contracts as TypeScript, and Rask's browser layer. Gitignored — do not edit. |
...
Two processes. The browser talks to Analog's own dev server on
http://localhost:5173, which proxies `/_rask` back to the host — so hot module replacement is
