...
A SolidStart front end on an ASP.NET host: one project, one container, one port.
...
| `Company.RaskServer/client/` | The SolidStart app, as `create-solid` scaffolds it, plus the config Rask adjusts. |
...
Two processes. The browser talks to SolidStart's own dev server on
