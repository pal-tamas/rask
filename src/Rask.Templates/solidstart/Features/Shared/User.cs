...
// the roles. It is an aggregate like any other, so read it with User.FindAsync(id) and change it with
// User.UpdateAsync(id, u => u.Rename(name)); register, sign in and reset passwords through Rask's IAuth.
