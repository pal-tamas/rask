...
A Nuxt front end on an ASP.NET host: one project, one container, one port.
...
| `Company.RaskServer/client/` | The Nuxt app, as `nuxi` scaffolds it, plus the config Rask adjusts. |
...
Two processes. The browser talks to Nuxt's own dev server on
