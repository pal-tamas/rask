...
// serializer reads Children (EnterChildrenScope), so the initial render reflects the bound/controlled value
// without a round-trip. `Select.Bind(() => model.Country)` infers T from the expression.

/// <summary>
///     A drop-down or list box of <c>option</c> children, optionally grouped by <c>optgroup</c>, typed by the
///     value it picks: <c>Bind</c> writes the chosen value straight back to your model.
/// </summary>
public sealed partial class HTMLSelectElement<T> : HTMLSelectElement, IFormControl<T>
{
    ...
    /// <summary>The name submitted with the form. A bound select defaults it to the bound member's name.</summary>
    ...
    private static IEnumerable<Component?> MarkSelected(IEnumerable<Component?> children, Selection current)
    {
        ...
        {
            if (c is HTMLOptionElement opt)
            {
                list.Add(MarkOption(opt, current));
            }
            else if (c is HTMLOptGroupElement og)
            {
    ...
    private static HTMLOptGroupElement MarkOptgroup(HTMLOptGroupElement og, Selection current)
    {
        ...
        var newChildren = og.Children.Select(c =>
            c is HTMLOptionElement o ? MarkOption(o, current) : c).ToArray();
