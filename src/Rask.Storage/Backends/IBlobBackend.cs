...
internal interface IBlobBackend
{
    ...
    /// <summary>Opens the object from <paramref name="offset"/> to its end, or returns <c>null</c> when it does not exist.</summary>
    Task<Stream?> OpenReadAsync(string key, long offset, CancellationToken cancellationToken);
