...
///     </para>
///     <para>
///         Everything that makes a component TypeScript-backed lives here rather than in generated
...
///         extends something else cannot be TypeScript-backed. That is deliberate: chrome in Rask comes
...
///     </para>
/// </remarks>
public abstract partial class TypeScriptComponent : Component
{
    ...
    ///     </para>
    /// </remarks>
    public TypeScriptHydration? Hydration { get; set; }
    ...
    protected sealed override string? TagName => TypeScriptDefaults.HostTag;
    ...
    protected override Component? HeadAssets =>
        Script.Src(TypeScriptDefaults.RuntimeScriptUrl).Type("module");
    ...
    /// </summary>
    /// <remarks>
    ///     Sealed rather than merely overridden, so that writing a <c>Render()</c> on a TypeScript
    ...
    {
        AppendAttr(sb, TypeScriptDefaults.NameAttribute, ComponentName);
        AppendAttr(sb, TypeScriptDefaults.ModuleAttribute, Module);
        AppendAttr(sb, TypeScriptDefaults.RuntimeAttribute, Runtime);

        if (Hydration is { } declared && TypeScriptDefaults.Wire(declared) is { } hydrate)
        {
            AppendAttr(sb, TypeScriptDefaults.HydrateAttribute, hydrate);
        }
        ...
        AppendAttr(sb, TypeScriptDefaults.PropsAttribute, WriteProps());
    }
}
