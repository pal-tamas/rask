// rask-ts Lit adapter, vendored from Rask.TypeScript.
//
 ...
 */
export function litComponent(tag: string): TypeScriptAdapter<HTMLElement> {
  return {
    mount(element, props, slots) {
      ...
      for (const name of Object.keys(slots ?? {})) {
        const fragment = (slots as TypeScriptSlots)[name]
        if (name !== 'default') {
 ...
 */
function assign(node: HTMLElement, props: TypeScriptProps): void {
