// rask-ts adapter contract, vendored from Rask.TypeScript.
//
// Everything Rask needs from React, Lit, Vue, Svelte or Angular fits in three functions. That is what
// makes the second and third runtimes cheap once the first exists — and why rask-ts.js imports
...
export type TypeScriptProps = Record<string, unknown>
 ...
 */
export type TypeScriptSlots = Record<string, DocumentFragment>
 ...
 */
export interface TypeScriptAdapter<THandle = unknown> {
  /**
   ...
   */
  mount(element: Element, props: TypeScriptProps, slots?: TypeScriptSlots): THandle
   ...
   */
  update?(handle: THandle, props: TypeScriptProps): THandle | void
