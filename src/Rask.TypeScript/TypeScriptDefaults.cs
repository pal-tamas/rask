...
///     and <c>rask-ts.js</c> that finds and mounts it.
/// </summary>
/// <remarks>
...
/// </remarks>
public static class TypeScriptDefaults
{
    /// <summary>
    ...
    /// </remarks>
    public const string HostTag = "rask-ts";
    ...
    /// </summary>
    public const string RuntimeScriptUrl = "/_content/Rask.TypeScript/rask-ts.js";

    /// <summary>The wire spelling of a hydration policy, or null for the default.</summary>
    /// <remarks>
    ///     Null for <see cref="TypeScriptHydration.Load" /> so the common case writes no attribute at all —
    ///     the client already treats a missing policy as "mount when the chunk is ready".
    /// </remarks>
    public static string? Wire(TypeScriptHydration hydration) => hydration switch
    {
        TypeScriptHydration.Idle => "idle",
        TypeScriptHydration.Visible => "visible",
        TypeScriptHydration.None => "none",
        _ => null,
    };
}
