...
public sealed partial class App(NotesReady ready, BrowserSqliteOwnership ownership, IServiceProvider services) : Component
{
    ...
    /// <inheritdoc />
    protected override async Task OnMountAsync() => _owner = await ownership.Resolved;
    ...
    protected override Component? Render()
    {
        ...
        return Main.Class("notes")[
            ...
            Form.Model(_draft).Key(_form).OnValidSubmit(AddAsync).Class("notes-form")[submitting => [
                ...
                Ui.Button.Id("add-note").Type(Ui.ButtonType.Submit).Tone(Ui.Tone.Primary)
                    .Disabled(submitting).Loading(submitting)["Add note"],
