...
At the last edit the highest allocated here was RASK085 (RASK077–080 package islands; RASK081 the
retired generated writes, RASK082/083 the generated form model and RASK084/085 the model-state analyzers, in `Rask.Batteries.Generators`), so the next
free is **RASK086** — and note this line once said RASK076 while RASK076 was already taken, and the form
...
```bash
grep -rhoE '"RASK[0-9]{3}"' src/ | sort -u | tail   # ALL of src/ — the list of projects grows
```
