...
| daisyUI | **Vendored, not a version string:** `src/Rask.Ui/Styles/vendor/daisyui.mjs` is the standalone bundle itself. Read the current version from `var version = "…"` inside it; the same number is restated in the `@plugin` comment in `src/Rask.Ui/Styles/ui.css`, so **bump both**. Bumping means re-downloading the pinned release asset (`daisyui.mjs` from daisyUI's releases page), not an npm install — this project deliberately has no `package.json`, because Tailwind resolves `@plugin "daisyui"` the Node way and the standalone engine carries no package tree. Nothing else watches this pin: `dependabot.yml` declares no npm ecosystem at all. After a bump run the unit gate — `UiClassNamesTests` fails if the new bundle stopped emitting a class the kit writes, which is the silent failure mode (the component renders unstyled and the build stays green). |
| Preact fixture | `tests/Rask.External.Tests/Rask.External.Tests.csproj` → `RaskPreactFixtureVersion` and `RaskHappyDomFixtureVersion`. Exact versions, npm-installed into `obj/` for `PreactAdapterTests`, which is the only coverage the Preact adapter has. Bump both, delete `tests/Rask.External.Tests/obj/preact-fixture` (the install is stamped) and run the unit gate; a preact major that changed `render()`/unmount semantics fails there, which is the point of the pin. |
